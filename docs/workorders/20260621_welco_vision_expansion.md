...
> ⚠️ **브랜딩 정정 (2026-06-22, 회장 최종 결정 — 이 문서의 일부를 무효화):**
> 이 문서 아래 일부 항목(브랜드 통일 등)은 "WelKor → Welco" 방향으로 적혀 있으나 **이는 폐기됨.**
> **정식 명칭 = "Welcome to Korea" → 줄여서 "웰코 / WelKor".** 즉 최종 방향은 **welco → WelKor**(반대).
> 코드·폴더·Vercel 프로젝트·공개 URL 전부 `welkor`로 통일 완료(공개 URL = https://welkor.vercel.app). 브랜드 관련 체크는 이 정정을 따른다.
